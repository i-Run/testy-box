...
import static fr.irun.testy.jooq.WithFlywaySchemaHistory.DEFAULT_TABLE_NAME;
...
class WithFlywaySchemaHistoryDefaultTableTest {
    ...
    void should_have_inserted_versions_in_default_table(@DbCatalogName String schema) {
        final WithFlywaySchemaHistory.FlywayTable historyTable = new WithFlywaySchemaHistory.FlywayTable(DEFAULT_TABLE_NAME, schema);
        final List<FlywayVersion> actual = dsl.selectFrom(DEFAULT_TABLE_NAME)
                .fetchStream()
